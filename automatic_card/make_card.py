        ...
        for grade in Grade.objects.all():
            ...
            for teacher in grade.teacher_set.all():
                ...
                for teacher_subject in teacher.subject_set.all():
                    ...
                    if teacher_subject.subject_name not in subject_teacher_dict:
                        ...
                        subject_teacher_dict[
                            teacher_subject.subject_name].append(
                ...
                for teacher_rule in teacher.teacherrule_set.all():
                    if teacher_rule.item == -1:
                        teacher_card[teacher.teacher_name][
                            teacher_rule.column_card.column_index][
                            teacher_rule.row_card.row_index] = ''
            ...
            for class_normal in grade.classnormal_set.all():
                ...
                for class_rule in class_normal.classnormalrule_set.all():
                    class_card[class_normal.class_name][
                        class_rule.column_card.column_index][
                        class_rule.row_card.row_index] = class_rule.item
            ...
            for key in subject_teacher_dict:
                ...
                if key in class_subject_number:
                    teacher_class_number = int(class_subject_number[key] / len(
                        subject_teacher_dict[key]))
                    rest = class_subject_number[key] % len(
                        subject_teacher_dict[key])
                    ...
                    for index in range(rest):
                        teacher_list[key].append(
                            subject_teacher_dict[key][index])
            for key in subject_class_dict:
                if key in teacher_list:
                    for class_index in range(len(subject_class_dict[key])):
                        for index in range(len(class_subject_dict[
                                                   subject_class_dict[key][
                                                       class_index]])):
                            if class_subject_dict[
                                subject_class_dict[key][class_index]][index][
                                'subject_name'] == key:
                                class_subject_dict[
                                    subject_class_dict[key][class_index]][
                                    index]['teacher'] = teacher_list[key][
                                    class_index]
            old_class_hour = {}
            for key in class_card:
                old_class_hour[key] = {}
                for old_subject_hour in class_subject_dict[key]:
                    old_class_hour[key][old_subject_hour['subject_name']] = \
                    old_subject_hour['subject_number']
                for column_key in class_card[key]:
                    for row_key in class_card[key][column_key]:
                        if class_card[key][column_key][row_key] in \
                                old_class_hour[key]:
                            old_class_hour[key][
                                class_card[key][column_key][row_key]] -= 1
            ...
            for key in class_subject_dict:
                ...
                for subject in class_subject_dict[key]:
                    ...
                    if teacher:
                        ...
                        for i in range(
                                old_class_hour[key][subject['subject_name']]):
                            random.shuffle(same_point)
                            current_point = same_point.pop()
                            teacher_card[teacher][current_point[0]][
                                current_point[1]] = ''
                            class_card[key][current_point[0]][
                                current_point[1]] = subject['subject_name']
                    else:
                        no_teacher_subject[
                            subject['subject_name']] = copy.deepcopy(
                            old_class_hour[key][subject['subject_name']])
            ...
            for key in class_subject_dict:
                ...
                for subject_hour in class_subject_dict[key]:
                    if subject_hour['teacher']:
                        teacher = Teacher.objects.get(Q(teacher_grade=grade), Q(
                            teacher_name=subject_hour['teacher']))
                ...
                for column_key in class_card[key]:
                    for row_key in class_card[key][column_key]:
                        ...
                        subject = Subject.objects.get(
                            subject_name=class_card[key][column_key][row_key])
                        try:
                            teacher = class_normal.teacher.get(subject=subject)
                        except BaseException:
                            teacher = class_normal.head_master
                        card = Card(str(uuid.uuid1()).replace('-', ''),
                                    row.row_id, column.column_id,
                                    subject.subject_id, teacher.teacher_id,
                                    class_normal.class_id)
                        card.save()
...
def get_teacher_card(request):
    ...
    grade = Grade.objects.get(grade_name=teacher_name.split('|')[0].
                              replace('教师', ''))
    teacher = Teacher.objects.get(Q(teacher_grade=grade),
                                  Q(teacher_name=teacher_name.split('|')[1]))
    items = []
